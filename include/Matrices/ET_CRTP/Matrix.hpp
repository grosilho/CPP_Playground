...
namespace LinAlg::Matrices::ET_CRTP
{
    ...
    {
        return Matrix<T>(LinAlg::Matrices::ET_CRTP::Constant<T>(rows, cols, value));
    }

    template <typename T>
    Matrix<T> Matrix<T>::Zero(int rows, int cols)
    {
        return Matrix<T>(LinAlg::Matrices::ET_CRTP::Zero<T>(rows, cols));
    }

    template <typename T>
    Matrix<T> Matrix<T>::Identity(int n)
    {
        return Matrix<T>(LinAlg::Matrices::ET_CRTP::Identity<T>(n));
    }
